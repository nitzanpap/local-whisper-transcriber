...
from config import BINARIES, CATALOGUE, Cancelled, Failed, settings
...
@lru_cache(maxsize=1)
def catalogue() -> dict:
    """The models this app knows how to describe, by filename.

    Generated by tools/gen_models.py, shipped beside the code, read once. It exists
    so that nobody has to know what a ggml quantisation is to choose how good their
    transcript will be — and so the ordering is a judgement rather than a file size.
    """
    try:
        found = json.loads(CATALOGUE.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return {}
    return {m["filename"]: {**m, "rank": n} for n, m in enumerate(found.get("models", []))}


def describe(path: str) -> dict:
    """What to say about a model file, from the catalogue if it is one we know."""
    known = catalogue().get(Path(path).name)
    if known:
        return {k: known[k] for k in ("id", "name", "description", "speed",
                                      "accuracy", "recommended", "rank")}
    # Something somebody put there themselves. Its own name, and no claims about
    # how good it is — a guess from the filename told people that large-v3-turbo
    # was the best model on the machine, which it is not.
    return {"id": Path(path).stem.removeprefix("ggml-"),
            "name": Path(path).stem.removeprefix("ggml-"),
            "description": "", "speed": None, "accuracy": None,
            "recommended": False, "rank": 999}


@lru_cache(maxsize=8)
def find_models(extra_dir: str = "") -> tuple[dict, ...]:
    """whisper.cpp models in the usual places, best choice first.

    Ordered by the catalogue's judgement rather than by size. Largest-first meant
    the interface preselected whatever was biggest, so a machine holding Small and
    Large v3 defaulted to three gigabytes of the slowest model for no reason but
    its file size.

    Cached per extra_dir, so saving a model from a new folder re-scans.
    """
    found: dict[str, int] = {}
    ...
    for directory in dirs:
        try:
            ...
            continue
    listed = [{"path": p, "size": size, **describe(p)} for p, size in found.items()]
    listed.sort(key=lambda m: (m["rank"], m["name"]))
    return tuple(listed)
